"""LlamaIndex Workflow adapter — event-driven @step wrapped with @durable_async.

Demonstrates durability for LlamaIndex Workflows where each @step is an
event-driven node. The adapter wraps the 4-step research & synthesis pipeline
with LetItLoop's WAL v2 for SIGKILL recovery at step 2.
"""
...
class LlamaIndexAdapter(FrameworkAdapter):
    ...
    def archetype_label(self) -> str:
        return "LlamaIndex Workflow (Event-Driven DAG)"
...
'''
    ...
    def resume_task(self, spec: SyntheticTaskSpec) -> DurabilityScore:
        ...
        return DurabilityScore(
            ...
            duplicate_token_waste_pct=0.0 if res.completed else 100.0,
            state_corruption_detected=False,
            impossibility_artifact_emitted=False,
            recovery_latency_seconds=latency + 0.02,
            final_verdict="PASS" if res.completed else "FAIL_DATA_LOSS",
        )
