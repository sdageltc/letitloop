...
def check_plan_quality(plan: Plan, workspace_root: str = "") -> List[PlanQualityWarning]:
    ...
    for c in plan.contracts:
        ...
        if not checks:
            warnings.append(
                PlanQualityWarning(
                    "contract has no acceptance checks",
                    severity="warning",
                    task_id=tid,
                )
            )
        ...
        if not outputs:
            warnings.append(
                PlanQualityWarning(
                    "contract has no outputs defined",
                    severity="error",
                    task_id=tid,
                )
            )
        ...
        if allow_paths:
            for out in outputs:
                ...
                if out_path and allow_paths:
                    allowed = any(
                        out_path.startswith(a.rstrip("/\\") + "/")
                        or out_path.startswith(a.rstrip("/\\") + "\\")
                        or out_path == a
        ...
        if objective.strip().lower() in generic_objectives:
            warnings.append(
                PlanQualityWarning(
                    f"generic objective: '{objective.strip()}'",
                    severity="warning",
                    task_id=tid,
                )
            )
    ...
    for c in plan.contracts:
        ...
        for dep in c.get("depends_on", []):
            if dep not in all_ids:
                warnings.append(
                    PlanQualityWarning(
                        f"dependency '{dep}' does not exist in plan",
                        severity="error",
                        task_id=tid,
                    )
                )
            if dep == tid:
                warnings.append(
                    PlanQualityWarning(
                        f"contract depends on itself: {tid}",
                        severity="error",
                        task_id=tid,
                    )
                )
    ...
    for c in plan.contracts:
        ...
        if checks and outputs:
            ...
            all_weak = all(
                ch.get("kind") in weak_kinds and ch.get("expected", "") in (".+", ".*", True, 1) for ch in checks
            )
            is_scratch = all(out.get("path", "").startswith("scratch/") for out in outputs)
            if all_weak and not is_scratch:
                warnings.append(
                    PlanQualityWarning(
                        "all acceptance checks are weak for non-scratch outputs",
                        severity="error",
                        task_id=tid,
                    )
                )
    ...
    for c in plan.contracts:
        ...
        if qc.get("required") and not qs:
            outputs = contract_dict.get("outputs", c.get("outputs", []))
            is_scratch = all(o.get("path", "").replace("\\", "/").startswith("scratch/") for o in outputs)
        ...
        for check in checks:
            ...
            if kind == "min_size":
                ...
                if not cmd:
                    warnings.append(
                        PlanQualityWarning(
                            "command check has empty command",
                            severity="error",
                            task_id=tid,
                        )
                    )
    ...
    for c in plan.contracts:
        ...
        if qs:
            ...
            if dims and isinstance(dims, dict):
                total_weight = sum(v for v in dims.values() if isinstance(v, (int, float)))
                if total_weight <= 0:
                    warnings.append(
                        PlanQualityWarning(
