...
from orchestrator.replanner import InspectResults, replan
from orchestrator.state import create_initial_state, save_state


def _setup_state(
    tmp_path, task_id, status, attempt=1, worker_results=None, failure_class="", scope_violations=None, crash_reason=""
):
...
class TestReplanEvidence:
    ...
    def test_split_on_escalated(self, tmp_path):
        ...
        run_dir = _setup_state(tmp_path, "t1", "ESCALATED", attempt=3, worker_results=[{"exit_code": 1}])
    ...
    def test_crash_retry_with_remaining_attempts(self, tmp_path):
        ...
        run_dir = _setup_state(
            tmp_path, "t1", "BLOCKED", attempt=1, failure_class="task_crashed", crash_reason="RuntimeError: OOM"
        )
        ...
        valid_contract = {
            ...
            "objective": "test",
            "worker": {"model": "test", "max_attempts": 3},
            "inputs": [],
            "outputs": [{"path": "scratch/t1_out.txt"}],
            "acceptance_checks": [
                {"id": "t1-chk", "kind": "file_exists", "path": "scratch/t1_out.txt", "expected": True}
            ],
    ...
    def test_exhausted_crash_splits(self, tmp_path):
        ...
        run_dir = _setup_state(
            ...
            "BLOCKED",
            attempt=1,
            failure_class="task_crashed",
            crash_reason="RuntimeError: OOM",
    ...
    def test_replan_rationale_attached(self, tmp_path):
        ...
        run_dir = _setup_state(tmp_path, "t1", "VERIFICATION_FAILED", failure_class="verifier_output_missing")
    ...
    def test_partial_failure_only_failed_tasks_replanned(self, tmp_path):
        ...
        _setup_state(tmp_path, "t1", "COMPLETE")
    ...
    def test_inspect_results_reads_crash_reason(self, tmp_path):
        run_dir = _setup_state(
            tmp_path, "t1", "BLOCKED", failure_class="task_crashed", crash_reason="RuntimeError: disk full"
        )
        inspector = InspectResults(run_dir)
        info = inspector.inspect_task("t1")
