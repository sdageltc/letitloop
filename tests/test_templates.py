...
def test_apply_research_template():
    contract = apply_template(
        "research",
        {
            ...
            "outputs": [{"path": "scratch/r1_out.txt"}],
        },
    )
...
def test_apply_implementation_template():
    contract = apply_template(
        "implementation",
        {
            "task_id": "impl1",
            "objective": "build Y",
            "outputs": [{"path": "scratch/impl1_out.txt"}],
        },
    )
...
def test_apply_code_generation_template():
    contract = apply_template(
        ...
        {
            "task_id": "cg1",
            "objective": "generate Z",
            "outputs": [{"path": "scratch/cg1_out.txt"}],
        },
    )
...
def test_apply_verification_template():
    contract = apply_template(
        "verification",
        {
            "task_id": "v1",
            "objective": "verify Z",
            "outputs": [{"path": "scratch/v1_out.txt"}],
        },
    )
...
def test_apply_aggregation_template():
    contract = apply_template(
        "aggregation",
        {
            "task_id": "a1",
            "objective": "aggregate",
            "outputs": [{"path": "scratch/a1_out.json"}],
        },
    )
