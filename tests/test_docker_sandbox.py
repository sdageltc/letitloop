...
def test_docker_run_argv_construction(tmp_path):
    ...
    ws_docker = worker_adapters._docker_host_path(ws_abs)
    assert f"{ws_docker}:/workspace:ro" in volumes
    src_host = worker_adapters._docker_host_path(os.path.normpath(os.path.join(ws_abs, "src")))
    docs_host = worker_adapters._docker_host_path(os.path.normpath(os.path.join(ws_abs, "docs")))
    ...
    staged = os.path.join(str(ws), "scratch", "docker_instructions_task_docker_argv.txt")
...
def test_docker_volumes_root_rw_when_scope_covers_root(tmp_path):
    ...
    ws_docker = worker_adapters._docker_host_path(os.path.abspath(str(tmp_path)))
    assert f"{ws_docker}:/workspace:rw" in volumes
    assert not any(v.endswith(":/workspace:ro") for v in volumes)


def test_docker_host_path_normalizes_drive_colon():
    assert worker_adapters._docker_host_path("C:\\ws\\sub") == "/c/ws/sub"
    assert worker_adapters._docker_host_path("/opt/ws") == "/opt/ws"
